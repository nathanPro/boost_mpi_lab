...
int main(int argc, char **argv) {
    ...
    if (cpu)
        run_in_cpu(map.data(), c[0], c[1], delta, lo, hi, width, height, threads);
    else
        run_in_gpu(map.data(), c[0], c[1], delta, lo, hi, width, height, threads);
    ...
    if (world.rank() != 0)
        ...
        for (int i = 1, lo = lines_per_proc; i < world.size();
             i++, lo += lines_per_proc) {
            std::vector<png_byte> msg;
            world.recv(i, 0, msg);
            std::copy(begin(msg), end(msg),
                      next(begin(map), 4 * lo * width));
        }
